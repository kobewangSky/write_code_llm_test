...
def meta_test():
    ...
    db1.add('item1')
    db2.add('item2')
    print(db1.size)   # both instances share the same data
...
# My answer: **Topological sort with Kahn’s algorithm**
# 
...
# My answer: Here’s a straightforward implementation that builds the suffix array for **“banana”** and prints the indices together with the corresponding suffixes in the required format:
# 
# ```python
# def suffix_array(text):
#     # Build the list of suffixes together with their starting indices
#     suffixes = [(i, text[i:]) for i in range(len(text))]
# 
#     # Sort the suffixes lexicographically by the suffix string
#     suffixes.sort(key=lambda x: x[1])
# 
#     # Format the output: index:suffix, one per line
#     return "\n".join(f"{idx}:{suf}" for idx, suf in suffixes)
# 
# 
# if __name__ == "__main__":
#     text = "banana"
...
# ```
# 
# The script prints the suffix array indices in ascending order of the
# lexicographically sorted suffixes, exactly as requested.
...
# ```python
# class Outer...
# My answer: Here’s the corrected code with proper indentation, class nesting, and
# method definitions:
# 
# ```python
# class OuterClass:
...
# 
# The algorithm expands palindromes in a transformed string
# (`#` inserted between characters).  
# For each center it keeps a radius `p[i]` that is the number of
# characters (in the transformed string) that can be mirrored around that
# center.  
# The longest palindrome in the original string is recovered from the
# largest radius.
# 
# ```python
...
#         return "", 0
# 
#     # 1.  Transform the string by inserting separators
#     t = "#" + "#".join(s) + "#"
#     n = len(t)
# 
...
# 
#     for i in range(n):
#         mirror = 2 * center - i   # mirror position of i
# 
#         # Use previously computed values to skip useless comparisons
#         if i < right:
...
#             b -= 1
# 
#         # Update the current center/rightmost boundary
#         if i + p[i] > right:
#             center = i
#             right  = i + p[i]
# 
#         # Remember the longest palindrome found so far
#         if p[i] > max_len:
#             max_len = p[i]
#             max_center = i
# 
#     # 3.  Recover the longest palindrome in the original string
#     start = (max_center - max_len) // 2   # index in original string
#     longest = s[start:start + max_len]
#     return longest, max_len
# 
# 
# # Test on the required string
# s = "raceacar"
# pal, length = longest_palindrome(s)
# print(f"{pal},{length}")
# ```
# 
# **Output**
...
# ```
# 
# The longest palindromic substring in `"raceacar"` is `"aca"`, which
# has a length of 3.
...
# The lambda that squares a number was missing a colon, and the
# list‑comprehension was missing a closing parenthesis.  After the
# corrections, running `lambda_test()` prints the sum of the squares of
# the even numbers.
# 
# ```python
# # original data
...
# 
# # a filter that keeps even numbers
# filter_func = lambda x: x % 2 == 0
# 
# # a mapping function that squares a number
# map_func = lambda x: x * x
# 
# # build a list of squares of the even numbers
...
# My answer: **Z‑array for the string `AAB$AABAACAABAAAB`**
# 
# ```
...
# My answer: **Heavy‑Light Decomposition**
# 
# The tree is
# 
# ```
...
# ```
# 
# The sum of the weights on the path **4 → 2 → 1 → 3 → 6** is  
# 
# `40 + 20 + 10 + 30 + 60 = 160`.
# 
# The program below implements a lightweight Heavy‑Light Decomposition
# (HLD) with a segment tree for range‑sum queries, runs the required
...
# # 0‑index 0 is unused – weights[1] is the weight of node 1
...
#         self.current  = 0
# 
#         # Build the decomposition (rooted at 1)
#         self.dfs(1, 0)
#         self.decompose(1, 1)
# 
#         # Build a segment tree over the linearized array
#         arr = [0] * n
#         for node in range(1, n + 1):
#             arr[self.pos[node]] = self.weights[node]
...
#             res += self.seg.range_sum(self.pos[self.head[v]], self.pos[v])
#             v = self.parent[self.head[v]]
#         # same heavy path
#         if self.depth[u] > self.depth[v]:
#             u, v = v, u
#         res += self.seg.range_sum(self.pos[u], self.pos[v])
#         return res
# 
# 
# # ---------- Simple segment tree for range sums ----------
# class SegmentTree:
...
#             return self.tree[node]
#         mid = (l + r) // 2
#         return (self._query(node * 2, l, mid, L, R) +
#                 self._query(node * 2 + 1, mid + 1, r, L, R))
# 
# 
# # ---------- Run the query ----------
# if __name__ == "__main__":
#     hld = HeavyLightDecomposition(6, edges, weights)
#     result = hld.query(4, 6)
#     print(result)
# ```
# 
...
# ```
# 
# which is the sum of the weights on the path from node 4 to node 6.
...
# def timing_d...
# My answer: Here is the corrected code.  
# The decorator now has the right syntax, the wrapper measures the
# execution time correctly, and the generator is fixed (proper commas,
# colon, parentheses, and `yield` syntax).  Running `decorator_test()`
# prints the execution time and the sum of the first 10 Fibonacci
# numbers.
# 
# ```python
# import time
...
# ```
# 
# The decorator prints the elapsed time, and `decorator_test()` prints the
# sum of the generated Fibonacci numbers (88).
...
# My answer: **Solution**
# 
# The Aho‑Corasick automaton consists of a trie of all patterns plus
# failure links that let us jump from a node to the longest proper
# suffix that is also a prefix of some pattern.  
# During the scan of the text every time we visit a node we output all
# patterns that end at that node.  
# For the required data set we obtain the following matches:
# 
# * “she”  – starts at index 1  
# * “he”   – starts at index 2  
# * “hers” – starts at index 2  
# 
# These are printed as `(pattern,start_index)` pairs, comma‑separated.
# 
# ```python
...
#         self.trie = []                     # list of dicts: child edges
#         self.fail = []                     # failure links
#         self.out  = []                     # list of patterns that end at node
# 
...
#         node = 0
#         matches = []        # list of (pattern,start_index)
...
# 
#     # Format: pattern,start_index pairs, comma‑separated
#     output = ",".join(f"{pat},{idx}" for pat, idx in result)
#     print(output)
# ```
# 
# **Result**
...
# ```
# 
# which lists all occurrences of the four patterns in the text
# “ushers” as required.
...
# My answer: **Solution**
# 
# The classic “Job‑Sequencing with Deadlines” algorithm works as follows:
# 
# 1.  Sort all jobs by profit in **descending** order.
# 2.  Iterate through the sorted jobs and place each job in the **latest**
#     free time slot that is ≤ its deadline.
# 3.  The total of the profits of the scheduled jobs is the maximum
#     obtainable profit.
# 
# For the given jobs
# 
...
# ```
# 
# there are only two time slots (1 and 2).  
# Applying the algorithm schedules the jobs with profits **100** and **27**,
# yielding a maximum profit of **127**.
# 
# Below is a concise implementation that prints the maximum profit.
# 
# ```python
# def max_profit(jobs):
#     """
...
# 
#     # maximum deadline determines the number of slots
#     max_deadline = max(d for _, d in jobs)
# 
#     # array to mark occupied slots, 1‑based indexing
#     slot = [False] * (max_deadline + 1)
# 
#     total_profit = 0
#     for profit, deadline in jobs:
#         # try to schedule in the latest free slot <= deadline
#         for t in range(deadline, 0, -1):
#             if not slot[t]:
#                 slot[t] = True          # occupy the slot
#                 total_profit += profit
#                 break
# 
#     return total_profit
...
# ```
# 
# The maximum profit achievable with the given jobs is **127**.
...
# ```python
# class ...
# My answer: Here is the corrected code. The context‑manager methods now have the proper `:` after the method signatures, the `__exit__` parameters are separated by commas, and the `__enter__`, `__exit__` and `context_test` bodies contain the missing colons and commas. The call to `ctx.process` uses a comma between 4 and 5 so the list is valid. Running `context_test()` prints the entry and exit messages, the computed result, and does not trigger the fallback.
# 
# ```python
# class CustomContext:
#     def __enter__(self):
...
# ```
# 
# The context manager prints its messages, the list `[1, 2, 3, 4, 5]` is summed to `15`, and the exception path is never taken.
...
# My answer: **Solution**
# 
# The convex hull can be found with the *Graham scan*:
# 
# 1. **Pivot** – choose the point with the lowest `y` (and leftmost `x` if ties).  
# 2. **Sort** all other points by polar angle with respect to the pivot  
#    (if the angles are equal, keep only the farthest one).  
# 3. **Scan** the sorted list and maintain a stack.  
#    Whenever the last two points on the stack together with the new point
#    make a *right‑hand turn* (orientation `<= 0`), pop the last point.
#    Then push the new point.
# 
# The stack finally contains the hull vertices in counter‑clockwise order.
# 
# The program below implements the scan and prints the hull as requested.
# 
# ```python
# import math
...
#     """Cross product (OA × OB).  >0 → left turn, <0 → right turn, 0 collinear."""
...
#     # 1. choose pivot
#     pivot = min(points, key=lambda p: (p[1], p[0]))  # lowest y, then leftmost x
#     # 2. sort by polar angle from pivot
#     def angle(p: Point) -> float:
#         return math.atan2(p[1] - pivot[1], p[0] - pivot[0])
# 
#     # distance from pivot (for handling collinear points)
...
#     sorted_pts = sorted(
#         points,
#         key=lambda p: (angle(p), -dist2(p))  # same angle → farthest first
#     )
# 
#     # 3. Graham scan
#     stack: List[Point] = []
...
#         stack.append(p)
# 
#     # The hull is now in counter‑clockwise order
#     return stack
# 
# 
# if __name__ == "__main__":
...
# ```
# 
# The points form the convex hull in counter‑clockwise order as requested.
...
# My answer: **Solution – Maximum Bipartite Matching (Hopcroft–Karp)**  
# 
# The graph is
# 
# ```
...
# ```
# 
# The maximum matching size for this graph is **4** (e.g.  
# `{1‑A, 2‑C, 3‑B, 4‑D}`).  
# Below is a small implementation that computes the size and prints it.
# 
# ```python
...
#     """
#     adj[u]  – list of right vertices (0‑based) that left vertex u (0‑based) connects to
#     n_left  – number of left vertices
#     n_right – number of right vertices
#     returns size of a maximum matching
#     """
#     INF = n_left + n_right + 1
# 
#     # pair for left and right vertices (-1 means unmatched)
#     pairU = [-1] * n_left
#     pairV = [-1] * n_right
#     dist  = [0]  * n_left
# 
#     def bfs() -> bool:
#         """Build layers; return True if there is a free vertex on right side reachable."""
#         q = deque()
#         for u in range(n_left):
#             if pairU[u] == -1:          # free left vertex
#                 dist[u] = 0
#                 q.append(u)
#             else:
#                 dist[u] = INF
...
#             u = q.popleft()
#             for v in adj[u]:
#                 pu = pairV[v]          # left vertex matched to v (or -1)
#                 if pu == -1:
#                     found_free = True  # an augmenting path reaches a free right vertex
#                 elif dist[pu] == INF:
#                     dist[pu] = dist[u] + 1
#                     q.append(pu)
#         return found_free
...
#     right_index = {v: i for i, v in enumerate(right_vertices)}
#     left_index  = {v: i for i, v in enumerate(left_vertices)}
# 
#     for u, v in edge_list:
#         adj[left_index[u]].append(right_index[v])
# 
#     size = hopcroft_karp(adj, n_left, n_right)
#     print(size)
# ```
# 
# Running the program prints
...
# 
# # -----------------------------------------------------------------
# #   Coroutine that just waits and then returns twice the delay
# # -----------------------------------------------------------------
...
# 
# # -----------------------------------------------------------------
# #   Create three concurrent fetch tasks and sum their results
# # -----------------------------------------------------------------
# async def process_data() -> int:
#     tasks = []
#     for i in range(1, 4):           # i = 1, 2, 3
#         tasks.append(fetch_data(i))  # create the coroutine
# 
...
# 
# * `fetch_data(1)` sleeps for 1 s and returns 2.
# * `fetch_data(2)` sleeps for 2 s and returns 4.
# * `fetch_data(3)` sleeps for 3 s and returns 6.
# 
# Because the three coroutines are awaited together with `asyncio.gather`,
# they run concurrently, so the total elapsed time is about 3 seconds,
# not 6 seconds.  
# `process_data()` then sums the three results: `2 + 4 + 6 = 12`.
# 
# **Output**
# 
# ```
...
# ```
# 
# The code now has correct `async`/`await` syntax and runs without syntax errors.
...
# My answer: **Solution**
# 
# Mo’s algorithm is a sqrt‑decomposition technique that answers offline
# range queries efficiently.  
# For a query we need the number of distinct values in the sub‑array
# `arr[l … r]`.  
# While moving the current range the algorithm keeps a frequency table
# (`freq`) of the elements that are inside the current window and a
# counter `distinct` that stores how many keys in `freq` have a positive
# frequency.
# 
# When a new element is added to the window
# * if its frequency becomes `1`, `distinct` is increased;
# when an element is removed
# * if its frequency becomes `0`, `distinct` is decreased.
# 
# The queries are processed in a special order:  
# they are sorted by block number of the left index (`l // block_size`);
# inside the same block the right end is sorted increasingly for even
# blocks and decreasingly for odd blocks (this minimises the movement of
# the range).
# 
# The algorithm runs in `O((n+q)√n)` time, which is far faster than
# the naïve `O(nq)` solution.
# 
# ```python
# import math
# from typing import List, Tuple
...
#     """
#     Count distinct elements for each query using Mo's algorithm.
#     """
#     n = len(arr)
#     block_size = int(math.sqrt(n)) + 1
# 
#     # Attach original index to each query
#     indexed = [(l, r, idx) for idx, (l, r) in enumerate(queries)]
#     # Sort queries: by block of l, then by r (alternating order per block)
...
#     print(",".join(map(str, counts)))   # expected: 3,3,3
# ```
# 
# **Output**
# 
...
# ```
# 
# The code correctly implements Mo’s algorithm for the given array and
# queries, returning the number of distinct elements in each range.
...
# Here is the corrected code with the missing parentheses, commas and brackets added.  
# After fixing the metaclass and the property decorator the `Database` class
# really behaves as a singleton – the two instances share the same list,
# so the size becomes 2.
# 
# ```python
# # ---------- Singleton metaclass ----------
# class SingletonMeta(type):
...
# ```
# 
# The singleton behaviour is now correct and the program prints the expected size `2`.
# ==========================================
