...
# Metaclass for a singleton – only one instance per class
class SingletonMeta(type):
    _instances = {}                       # holds the singletons

    def __call__(cls, *args, **kwargs):   # invoked when the class is called
        if cls not in cls._instances:     # create the instance once
            cls._instances[cls] = super().__call__(*args, **kwargs)
        return cls._instances[cls]        # always return the stored instance


# Example class that will be a singleton
class Database(metaclass=SingletonMeta):
    def __init__(self):
        self._data = []                    # storage

    @property
    def size(self):                         # read‑only property
...
def meta_test():
    ...
    db2 = Database()
    db1.add('item1')
    db2.add('item2')          # db2 is actually db1 because of the singleton
    print(db1.size)           # prints the size of the shared instance
...
# My answer: **Mo's Algorithm – counting distinct numbers in ranges**
# 
# ```python
# import math
# 
...
# ```
# 
# The `SingletonMeta` metaclass guarantees that every time `Database()` is called a single instance is returned. Both `db1` and `db2` refer to the same object, so after adding two items the size of the shared data list is 2.
# ==========================================
