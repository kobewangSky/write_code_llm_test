...
# 作為一名油漆工，我經常把「尋找最大色調區塊」比作在一大幅牆面裡挑出最能映照光線的區域。當我在面對病痛、時間緊迫的時候，這種「聚焦」的思維也能幫助我把有限的精力放在最有價值的工作上——不管是最後一次塗底漆，還是最後一次把光線映照進客戶的客廳。這次的題目就像是挑選最明亮的一段油漆線條，我用 Kadane 演算法去找到「最大子陣列和」，相當於在整個顏料庫裡找出最能提升整體亮度的那一段顏料。
# 
