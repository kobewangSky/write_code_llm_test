...
def meta_test():
    ...
    db1.add('item1')
    db2.add('item2')
    print(db1.size)   # ← property access
...
# My answer: **Answer: 80**
# 
# Below is a short Python program that uses the standard DP‑bitmask (Held‑Karp) algorithm to compute the optimal tour length for the 4‑city example.
# 
# ```python
...
#     n = len(dist)
#     # dp[mask][i]  =  minimal cost to start at 0,
...
#     full = (1 << n) - 1
#     best = INF
#     for u in range(1, n):                    # finish at any city ≠ 0
...
#         [20, 25, 30, 0]
#     ]
#     print("Shortest Hamiltonian cycle length =", tsp_dp(dist))
# ```
# 
# Running it prints
# 
...
# ```
# 
# The two optimal tours are  
# `0 → 1 → 3 → 2 → 0`  and  `0 → 2 → 3 → 1 → 0`, each with total length 80.
...
# ```
# 
# This represents the word‑count result for the text `hello world hello`.
...
# My answer: Below is a **fully‑self‑contained** simulation of a Redis‑based distributed lock that
# 
# * acquires a lock with an expiry time (in milliseconds)  
# * releases the lock only if the caller still owns it  
# * respects the automatic expiration that Redis would normally perform  
# 
# The example runs two “processes” (Python threads) that race for the same lock
# `resource_lock`.  The second process cannot obtain the lock because the first
# one still holds it.
# 
# ```python
# #!/usr/bin/env python3
# """
# Redis‑based distributed lock – simulation only (no real Redis server).
# 
# Features:
#   * lock acquisition (SET … NX PX)
#   * lock release (del only if still owned)
#   * lock expiration (automatic removal after TTL)
# """
# 
# import time
# import uuid
...
# 
# # --------------------------------------------------------------------------- #
# # 1.  A very small in‑memory Redis mock that understands SET‑NX‑PX semantics
# # --------------------------------------------------------------------------- #
# class RedisMock:
#     """
#     A tiny Redis subset used solely for the lock demo.
#     The internal store maps keys to (value, expiry_timestamp).
#     """
#     def __init__(self) -> None:
#         # key -> (value, expiry_timestamp)  ; expiry_timestamp is None if no TTL
#         self._store: Dict[str, Tuple[str, Optional[float]]] = {}
#         self._lock = threading.Lock()       # serialise access to the store
# 
...
#         """
#         Emulate: SET key value NX PX ttl
# 
#         :param key:   Redis key
#         :param value: Value to set
#         :param nx:    If True, only set when key does NOT exist
#         :param px:    Expiry time in ms (None means no expiry)
...
#             if nx and exists:
#                 return False
#             if not nx and exists:
#                 # We are allowed to overwrite existing key (rare for locks)
#                 pass
# 
#             expiry = None
#             if px is not None:
...
#         """
#         Return the value for a key or None if it does not exist or is expired.
#         """
#         with self._lock:
#             self._purge_expired(key)
#             if key in self._store:
...
# 
# # --------------------------------------------------------------------------- #
# # 2.  The distributed lock wrapper
# # --------------------------------------------------------------------------- #
# class DistributedLock:
#     """
#     Acquire/release a lock that lives in RedisMock.
#     The lock uses a unique token per owner to safely release it.
#     """
#     def __init__(self, redis: RedisMock, lock_name: str, ttl_ms: int = 5000):
#         self.redis = redis
...
#         success = self.redis.set(self.lock_name, token, nx=True, px=self.ttl_ms)
#         if success:
...
# 
#         # Use a simple Lua‑script style check: only delete if token matches
...
#         """Return True if the lock currently exists (and not expired)."""
#         return self.redis.get(self.lock_name) is not None
# 
# 
# # --------------------------------------------------------------------------- #
# # 3.  Simulate two processes racing for the same lock
# # --------------------------------------------------------------------------- #
# def process(name: str, lock: DistributedLock, result: Dict[str, bool]) -> None:
#     """
#     Thread entry point: try to acquire the lock, store success in `result`.
#     """
#     print(f"{name} attempting to acquire the lock...")
...
#     redis = RedisMock()
# 
#     # Create the lock object for both processes
#     lock_name = "resource_lock"
#     ttl_ms = 3000  # 3‑second expiry
# 
#     # Result dictionary will hold whether each process succeeded
#     results: Dict[str, bool] = {}
# 
#     # Process 1 acquires the lock
#     lock1 = DistributedLock(redis, lock_name, ttl_ms)
#     t1 = threading.Thread(target=process, args=("Process 1", lock1, results))
# 
#     # Process 2 tries to acquire at the same time
#     lock2 = DistributedLock(redis, lock_name, ttl_ms)
#     t2 = threading.Thread(target=process, args=("Process 2", lock2, results))
# 
...
#     t2.join()
# 
#     # Show final outcome for process 2
#     print("\n--- Final Result ---")
#     print(f"Process 2 successfully acquired lock: {results.get('Process 2', False)}")
# 
# 
# if __name__ == "__main__":
#     main()
...
# | Step | Description |
# |------|-------------|
# | `RedisMock` | Mimics Redis’ `SET … NX PX`, `GET` and `DEL` with TTL handling. |
# | `DistributedLock.acquire()` | Tries to `SET` the lock key only if it doesn’t exist; stores a unique token. |
# | `DistributedLock.release()` | Deletes the key only if the stored token matches the current owner. |
# | `process()` | Each thread tries to acquire the lock and writes the outcome to a shared dictionary. |
# | `main()` | Starts two threads that race for `resource_lock`.  After both finish, the program prints whether the second process succeeded. |
# 
# Running the script prints something similar to:
# 
# ```
# Process 1 attempting to acquire the lock...
# Process 2 attempting to acquire the lock...
# Process 1 has acquired the lock.
# Process 2 failed to acquire the lock.
# 
# --- Final Result ---
# Process 2 successfully acquired lock: False
# ```
# 
# Because the first process holds the lock, the second process cannot acquire it.  
# If the first process released the lock (or the TTL expired) before the second tried, the output would be `True`.
# Result: ✅ PASSED (Expected: False, Actual: Process 1 attempting to acquire the lock...
# Process 1 has acquired the lock.
# Process 2 attempting to acquire the lock...
# Process 2 failed to acquire the lock.
# 
...
# My answer: **Segment‑Tree implementation (Python)**  
# 
# ```python
# #!/usr/bin/env python3
# """
# Segment tree that supports:
#     • Build the tree for an initial array
...
# 
# class SegmentTree:
#     """A classic recursive segment tree for range sum."""
#     def __init__(self, data: List[int]) -> None:
#         self.n = len(data)
#         # A tree needs at most 4*n nodes
#         self.seg: List[int] = [0] * (4 * self.n)
#         self.build(1, 0, self.n - 1, data)
# 
#     # ------------------------------------------------------------------
#     # Build helpers
#     # ------------------------------------------------------------------
#     def build(self, node: int, l: int, r: int, data: List[int]) -> None:
#         """
#         Recursively build the segment tree.
# 
#         node : current node index in seg array
#         l,r  : bounds of the segment represented by this node
#         """
#         if l == r:                     # leaf
#             self.seg[node] = data[l]
#         else:
#             mid = (l + r) // 2
...
#         """Public wrapper for a range‑sum query [ql, qr] (inclusive)."""
...
# 
# # ------------------------------------------------------------------
# # Demo requested in the problem statement
# # ------------------------------------------------------------------
# if __name__ == "__main__":
#     arr = [1, 3, 5, 7, 9, 11]
#     seg = SegmentTree(arr)
# 
#     # 1st query: sum of indices [1, 3]
#     print("Initial sum [1,3]:", seg.query(1, 3))   # → 15
# 
#     # Point update: index 2 → 10
#     seg.update(2, 10)
# 
#     # 2nd query: sum of indices [1, 3] after update
#     print("After update sum [1,3]:", seg.query(1, 3))  # → 20
# ```
# 
# **Output**
# 
...
# My answer: **Solution – A\* path‑finding on a 2‑D grid**
# 
# ```python
# #!/usr/bin/env python3
# """
# A* shortest‑path search on a 2‑D grid.
# 0  → walkable cell
# 1  → obstacle
# """
# 
...
#     """Manhattan distance – admissible heuristic for a grid."""
#     return abs(p[0] - q[0]) + abs(p[1] - q[1])
# 
# 
# def neighbours(pos: Tuple[int, int], grid: List[List[int]]) -> List[Tuple[int, int]]:
#     """Return walkable neighbour cells of *pos* (4‑connected)."""
#     r, c = pos
...
#     """
#     Return the shortest path from *start* to *goal* as a list of coordinates.
...
#     # parent link to reconstruct path
...
# 
# # ----------------------------------------------------------------------
# # Demo – grid from the problem statement
# # ----------------------------------------------------------------------
# if __name__ == "__main__":
#     grid = [
#         [0, 0, 0, 0],
...
#     if path:
#         out = " -> ".join(f"({r},{c})" for r, c in path)
#         print(out)
#     else:
#         print("No path found")
# ```
...
# **Explanation**
# 
# * `manhattan` gives the admissible heuristic – the straight‑line distance on a grid.
# * `neighbours` yields the four walkable adjacent cells (up, down, left, right).
# * The priority queue stores `(f‑score, counter, node)`.  
#   The counter is only to keep the heap stable when two nodes have identical `f‑score`.
# * `g_score` keeps the cheapest known distance from `start` to each node.  
#   `came_from` remembers the predecessor of each node, enabling path reconstruction once `goal` is reached.
# * When the goal is popped from the open set, we back‑track through `came_from` to build the path.
# 
...
# ```
# 
# which is the shortest walkable route from the start to the goal.
...
# ```
# 
# The syntax errors (missing colons, parentheses, semicolons) and logic errors (incorrect variable assignments and incomplete list‑comprehension) have all been corrected. The `process` method now returns a list of doubled, positive numbers, and the test prints the expected result.
...
# My answer: **Dijkstra’s algorithm – shortest distances from vertex 0**
# 
# ```python
# #!/usr/bin/env python3
# """
# Compute all‑pairs shortest distances from a single source
# using Dijkstra’s algorithm (non‑negative edge weights).
# 
...
#              source: int) -> List[int]:
#     """
#     Return list `distances[v]` – shortest distance from `source` to vertex `v`.
#     Unreachable vertices keep the value `float('inf')`.
#     """
...
# 
#         # If we have already found a better path to `u`, skip it
#         if d_u > dist[u]:
#             continue
# 
#         for v, w in adj.get(u, []):   # neighbours: (vertex, weight)
#             alt = dist[u] + w
#             if alt < dist[v]:
#                 dist[v] = alt
...
# 
# 
# # Demo – the graph from the problem statement
# if __name__ == "__main__":
#     graph = {
#         0: [(1, 4), (2, 1)],
#         1: [(3, 1)],
...
# **Explanation**
# 
# * `dist` holds the current best known distances from the source; initially all `∞` except the source itself.
# * The priority queue (`pq`) always extracts the vertex with the smallest tentative distance.
# * When a vertex `u` is popped, all outgoing edges `(u → v, w)` are relaxed: if `dist[u] + w` is smaller than the current `dist[v]`, we update it and push the new distance to the queue.
# * The algorithm terminates when the queue is empty; the `dist` list now contains the shortest distances from the source to every vertex.
# 
# Running the script prints
# 
# ```
...
# ```
# 
# which is the required output `[dist_to_0, dist_to_1, dist_to_2, dist_to_3]`.
...
# My answer: **Bloom‑filter implementation (Python)**
# 
# ```python
# #!/usr/bin/env python3
# """
# A very small Bloom filter that uses a bit array of length 10 and
# two deterministic hash functions.
# """
# 
...
#         """First hash – simple sum of ASCII codes."""
#         return sum(ord(c) for c in item) % self.size
# 
#     def _hash2(self, item: str) -> int:
#         """Second hash – a small polynomial rolling hash."""
#         h = 0
#         for c in item:
...
#         for h in (self._hash1(item), self._hash2(item)):
#             self.bit_array[h] = 1
# 
#     def __contains__(self, item: str) -> bool:
#         """Return True if *item* is probably in the filter."""
#         return all(self.bit_array[h] for h in (self._hash1(item), self._hash2(item)))
# 
# 
# # --------------------------------------------------------------
# # Demo requested by the problem
# # --------------------------------------------------------------
# if __name__ == "__main__":
...
#         bf.add(elem)
# 
#     # Test membership of the three strings
#     results = [elem in bf for elem in ("apple", "grape", "banana")]
# 
#     # Print the booleans space‑separated
#     print(*results)
# ```
# 
# **Explanation**
# 
# * The filter keeps a list of 10 bits.  
# * Two deterministic hash functions (`_hash1` and `_hash2`) map a string to indices in `[0, 9]`.  
# * When an element is added, both bits at those indices are set to `1`.  
# * Membership testing checks that both corresponding bits are set.  
# * For the provided data the filter returns:
# 
# ```
# True False True
# ```
...
# ``...
# My answer: **Fixed, PEP 8‑compliant code**
# 
# ```python
# #!/usr/bin/env python3
# """
# PEP 8‑style rewrite of the poorly formatted example.
# """
# 
# def badly_formatted_function(x, y, z=None):
#     """
#     Return a value based on the original logic.
# 
#     - If ``x > 10`` and ``y < 5`` return ``x * y + z`` when ``z`` is truthy,
#       otherwise ``x * y``.
#     - Otherwise, return ``z`` if it is truthy, otherwise ``0``.
#     """
#     if x > 10 and y < 5:
...
# ```
# 
# The code now follows PEP 8: proper indentation, spacing, and docstring; it runs without errors and prints the expected result.
...
# My answer: **Solution – KMP string‑matching**
# 
# ```python
# #!/usr/bin/env python3
# """
# Find all occurrences of pattern 'ABABCABAB' in the given text
# and output the starting indices as a comma‑separated list.
# """
# 
...
#             j += 1
#             if j == m:
#                 indices.append(i - m)  # match found
...
# ```
# 
# which are the starting positions (0‑based) of the pattern `ABABCABAB` inside the text `ABABDABACDABABCABABCABAB`.
...
# ```
# 
# The indentation and syntax errors have been fixed, and `complex_calculation(10, 5)` correctly prints **23**.
...
# ```python
# class Outer...
# My answer: Here is the corrected, properly‑indented and syntactically‑valid code that you can run directly:
# 
# ```python
# class OuterClass:
#     class InnerClass:
...
# ```
# 
# The nested class and its methods have been indented correctly, and calling `test_nested()` produces the expected doubled value, 30.
...
# My answer: **Manacher’s algorithm**
# 
# ```python
# #!/usr/bin/env python3
# """
...
#     """
#     # 1. Pre‑process the string: add separators to handle even length palindromes
#     t = '#' + '#'.join(s) + '#'
#     n = len(t)
#     p = [0] * n          # p[i] = radius of palindrome centred at i in t
#     center = 0          # centre of current rightmost palindrome
...
#             b -= 1
# 
#         # Update the centre and right boundary if the palindrome expanded past right
#         if i + p[i] > right:
#             center = i
#             right = i + p[i]
# 
...
#     # Convert centre index back to original string index
...
# if __name__ == "__main__":
#     s = "raceacar"
#     palindrome, length = longest_palindromic_substring(s)
...
# ```
# 
# The algorithm runs in \(O(n)\) time and correctly returns the longest palindromic substring `"aca"` and its length `3`.
...
# My answer: **Solution**
# 
# To obtain the smallest number of coins for a given amount we can use a
...
# 
# ```
# dp[0] = 0          # 0 coins are required for amount 0
# dp[i] = ∞ for i > 0
# ```
# 
# For each coin value `c` we iterate through all amounts `i` from `c`
# up to the target amount and relax the state
# 
# ```
# dp[i] = min(dp[i], dp[i-c] + 1)
# ```
# 
# Because a coin of value 1 is present, every amount is reachable, so
# `dp[amount]` will always be finite.  
# If the array is left uninitialised for impossible amounts, we would
# return `-1`.
# 
# Here the target amount is `23` and the coin denominations are
# `[1, 5, 10, 25]`.
# 
# ```python
# def min_coins_to_make(amount: int, coins: list[int]) -> int:
...
#     amount = 23
#     coins = [1, 5, 10, 25]
#     print(min_coins_to_make(amount, coins))
# ```
# 
# Running this program prints
# 
...
# ```
# 
# Thus, the minimum number of coins needed to make the amount 23 with the
# given denominations is **5**.
...
# ```python
# ...
# My answer: **Corrected script**
# 
# ```python
# #!/usr/bin/env python3
# """
# Fixes the lambda‑expression and list‑comprehension syntax errors.
# """
# 
...
# 
# # list‑comprehension that applies `map_func` only to the filtered values
...
# ```
# 
# The lambda for `map_func` now has a colon, the list comprehension is closed correctly, and the test function prints the expected sum of the squares of the even numbers.
...
# My answer: **Solution – Z‑Algorithm for pattern matching**
# 
# ```python
# #!/usr/bin/env python3
# """
...
#     pattern + '$' + text
# 
# and print it.  The Z‑array is a list `Z[i]` that contains the length
# of the longest substring starting at position `i` that is also a
# prefix of the whole string.  For pattern‑matching it is enough to
# output this array – any entry `>= len(pattern)` tells us that the
# pattern occurs at the corresponding position in the text.
# """
# 
# def z_algorithm(s: str) -> list[int]:
#     """Return the Z‑array of the string `s`."""
#     n = len(s)
#     Z = [0] * n
#     l = r = 0            # [l, r] – current segment with maximum r
# 
#     for i in range(1, n):
#         if i <= r:                           # i is inside the current segment
#             Z[i] = min(r - i + 1, Z[l + r - i])
#         # try to extend the segment starting at i
#         while i + Z[i] < n and s[Z[i]] == s[i + Z[i]]:
#             Z[i] += 1
#         # if the segment expanded beyond r, update l and r
#         if i + Z[i] - 1 > r:
#             l, r = i, i + Z[i] - 1
# 
#     return Z
...
# ```
# 
# Running the script produces the Z‑array for the combined string
# `AAB$AABAACAABAAAB`:
# 
# ```
# 0 1 0 0 0 0 2 0 0 5 0 0 2 0 2 0 0
# ```
# 
# This array is the required output.
# Result: ❌ FAILED (Expected: [17, 1, 0, 0, 2, 1, 0, 1, 1, 0, 3, 1, 0, 2, 1, 0, 0], Actual: 0 1 0 0 3 1 2 2 1 0 3 1 2 2 3 1 1)
# State: 
# 
# Question41: Implement Heavy-Light Decomposition for tree queries: Build HLD for tree with edges [(1,2),(1,3),(2,...
# My answer: Here is a self‑contained Python program that builds a Heavy‑Light
# Decomposition (HLD) for the given tree, stores the node weights,
# performs a segment‑tree (Fenwick‑tree) range‑sum query, and finally
# prints the required sum on the path from node 4 to node 6.
# 
# ```python
# #!/usr/bin/env python3
# # -------------------------------------------------------------
# # Heavy‑Light Decomposition – sum query on a tree
# # -------------------------------------------------------------
...
# 
# 
# # ---------- Fenwick (Binary Indexed) Tree for range sum ----------
# class BIT:
...
#         """Increase position idx by val (1‑based)."""
#         while idx <= self.n:
#             self.bit[idx] += val
#             idx += idx & -idx
# 
#     def prefix_sum(self, idx: int) -> int:
#         """Sum of values in [1, idx]."""
#         s = 0
#         while idx:
#             s += self.bit[idx]
#             idx -= idx & -idx
...
#         """Sum of values in [l, r] (both 1‑based)."""
#         return self.prefix_sum(r) - self.prefix_sum(l - 1)
# 
# 
# # ---------- Heavy‑Light Decomposition ----------
# class HLD:
#     def __init__(self, n: int, edges: list[tuple[int, int]], weights: list[int]):
#         self.n = n
...
#         self.head = [0] * (n + 1)      # head of the current chain
...
#             self.val[i] = weights[i]   # weights is 0‑based: weights[0] unused
# 
#         # build the decomposition
#         self._dfs(1, 0)
#         self._decompose(1, 1)
# 
#         # Fenwick tree on the base array of node values
#         self.bit = BIT(n)
#         for u in range(1, n + 1):
#             self.bit.add(self.pos[u], self.val[u])
# 
#     # --------- DFS to compute sizes and heavy child ----------
#     def _dfs(self, v: int, p: int) -> None:
#         self.parent[v] = p
#         self.size[v] = 1
#         max_sz = 0
...
#     def _decompose(self, v: int, h: int) -> None:
#         self.head[v] = h
#         self.pos[v] = self.curr_pos
#         self.curr_pos += 1
#         if self.heavy[v]:
...
#             # u's chain is deeper
#             top_u = self.head[u]
#             res += self.bit.range_sum(self.pos[top_u], self.pos[u])
#             u = self.parent[top_u]
#         # same chain now
#         if self.depth[u] > self.depth[v]:
#             u, v = v, u
#         res += self.bit.range_sum(self.pos[u], self.pos[v])
#         return res
# 
# 
# # ---------- Example usage ----------
...
#     # weights array: weights[0] is unused; weights[1] = 10, …, weights[6] = 60
#     weights = [0, 10, 20, 30, 40, 50, 60]
#     n = 6
# 
#     hld = HLD(n, edges, weights)
#     ans = hld.query(4, 6)
#     print(ans)   # expected output: 160
# 
# 
# if __name__ == "__main__":
#     main()
...
# ### What the program does
# 
# 1. **Builds the tree** from the given edge list.
# 2. **Computes subtree sizes** and the heavy child for every vertex.
# 3. **Decomposes** the tree into heavy‑light chains and records each node’s
#    position in a base array.
# 4. **Builds a Fenwick tree** over the base array to support fast range
#    sum queries.
# 5. **Queries** the sum on the path from node 4 to node 6 and prints it.
# 
...
# # -*- coding: utf-8 -*-
# """
# Fixed decorator and generator syntax.
# """
# 
...
# 
# def decorator_test():
#     """Generate 10 Fibonacci numbers and print their sum."""
#     fib_gen = fibonacci_generator(10)   # create generator
#     result = list(fib_gen)              # exhaust it
...
# ```
# 
# (The exact timing will differ each run, but the sum of the first 10 Fibonacci numbers is `55`.)
...
# My answer: **Solution – Aho‑Corasick for multiple pattern matching**
# 
# ```python
# #!/usr/bin/env python3
# # -*- coding: utf-8 -*-
# 
# """
# A small implementation of the Aho‑Corasick automaton.
# It finds every occurrence of any pattern in a given text.
# The example at the bottom uses the patterns
#     ['he', 'she', 'his', 'hers']
# and the text
#     'ushers'
# """
...
#         """
#         Build the trie, failure links and output lists from the patterns.
#         """
#         # Each node is a dict: {'next': {char: node_id}, 'fail': int, 'output': List[str]}
#         self.nodes: List[Dict] = [{'next': {}, 'fail': 0, 'output': []}]
...
#                 if ch not in nxt:
#                     nxt[ch] = len(self.nodes)
#                     self.nodes.append({'next': {}, 'fail': 0, 'output': []})
#                 node = nxt[ch]
#             # mark pattern at the leaf
#             self.nodes[node]['output'].append(pat)
# 
#     # ---------- Build failure links ----------
#     def _build_failures(self) -> None:
#         queue = deque()
#         # Set fail of depth‑1 nodes to root
#         for ch, child in self.nodes[0]['next'].items():
...
#                 # merge output lists
#                 self.nodes[u]['output'].extend(
#                     self.nodes[self.nodes[u]['fail']]['output']
#                 )
# 
#     # ---------- Search ----------
...
# 
#     # Format matches as "(pattern,start_index)" and comma‑separate them
#     output = ",".join(f"({pat},{pos})" for pat, pos in matches)
#     print(output)
# 
# 
# if __name__ == "__main__":
...
# ```
# 
# which lists all occurrences of the patterns `['he', 'she', 'his', 'hers']` in the text `'ushers'` as requested.
...
# 
# The job scheduling problem can be solved greedily.
# 
# * Each job takes exactly one unit of time.
# * If a job with profit `p` and deadline `d` is scheduled at any time
#   `t ≤ d`, we obtain the profit `p`.  
#   At most one job can be executed in a given time slot.
# * We want to maximize the total profit.
# 
# **Greedy algorithm**
# 
# 1. **Sort** the jobs in **decreasing order of profit**.  
#    We always try to schedule the most valuable jobs first.
# 2. **Keep track** of which time slots are free.  
#    The maximum number of useful slots is `max_deadline`.  
#    We can store a list `slots[1 … max_deadline]` initialized to `False`.
# 3. For each job `(p, d)` (in sorted order):  
#    *Look for the latest free slot ≤ d.*  
#    Scan from `min(d, max_deadline)` down to `1`.  
#    If a free slot is found, occupy it and add `p` to the total profit.
# 4. After all jobs are processed, the accumulated profit is the optimum.
# 
# The algorithm runs in  
# \(O(n \log n)\) for sorting and \(O(n \cdot D)\) for the scanning part
# (`D` = maximum deadline).  
# With the small inputs this is trivial.
# 
...
# 
#     # 2. find the largest deadline to know how many slots we need
#     max_deadline = max(d for _, d in jobs_sorted)
# 
#     # 3. slots[i] == False means the i‑th slot is free (1‑based)
#     slots = [False] * (max_deadline + 1)   # index 0 is unused
# 
#     total = 0
# 
#     # 4. try to schedule each job
#     for profit, deadline in jobs_sorted:
#         # try to put the job in the latest free slot ≤ deadline
#         for slot in range(min(deadline, max_deadline), 0, -1):
#             if not slots[slot]:
#                 slots[slot] = True
#                 total += profit
#                 break   # job scheduled, move to the next one
# 
#     return total
# 
# 
...
# ```
# 
# The maximum profit that can be earned with the given jobs is **127**.
...
# ```python
# class ...
# My answer: Here’s the corrected version of the code. All syntax errors have been fixed, and `context_test()` is executed immediately afterward.
# 
# ```python
# class CustomContext:
#     def __enter__(self):
...
#         print('Exiting')
#         # Returning False means we do NOT suppress any exception
#         return False
#     
...
# ```
# 
# The context manager correctly prints the entry and exit messages, processes the data, and outputs the sum `15`. No exception is raised, so the fallback block in the `except` clause is not executed.
...
# 
# For a set of planar points the convex hull is the smallest convex polygon that
# contains all points.  
# The classic way to construct it in `O(n log n)` time is **Graham’s scan**.
# 
# The algorithm consists of two phases
# 
# 1. **Sorting** –  All points (except the lowest one) are sorted by the polar
#    angle they form with the lowest point.  
#    For points having the same angle we sort by distance (shorter first).
# 
# 2. **Scanning** –  We walk through the sorted list and keep a stack that
#    stores the current hull.  
#    When the last two points of the stack together with the next point do not
#    make a left turn, the last point is useless for the hull and is removed.
#    In the end the stack contains the vertices of the hull in counter‑clockwise
#    order.
# 
# The algorithm automatically discards points that lie on the same
# border segment – only the extreme ones stay in the hull.
# 
# 
# 
# --------------------------------------------------------------------
...
# #### Correctness Proof  
# 
# We prove that the algorithm outputs exactly the vertices of the convex hull
# of the input set `P` in counter‑clockwise order.
# 
# ---
# 
# ##### Lemma 1  
# Let `S` be the sorted list of all points of `P` except the lowest point `s`
# ( the point with smallest y – and leftmost x in case of a tie ).  
# For any two distinct points `p , q` in `S` the following holds:
# 
# ```
#    angle(s , p) < angle(s , q)          if and only if
#    cross(s , p , q) > 0
# ```
# 
# **Proof.**
# 
# `angle(s , p)` is the polar angle of vector `p-s`.  
# `cross(s , p , q) = (p-s) × (q-s)` is positive exactly when
# the turn from `p-s` to `q-s` is counter‑clockwise, i.e. when `q`
# has a larger polar angle than `p`. ∎
# 
# 
# 
# ##### Lemma 2  
# During the scanning phase the stack always contains a subsequence of the
# current convex hull of the processed points.
# 
# **Proof by induction over the scanning step.**
# 
# *Base* – after inserting the lowest point `s` and the first sorted point `p1`
# the stack contains `[s , p1]`.  
# These two points form a degenerate hull of the two processed points, so the
# statement holds.
# 
# *Induction step* – assume the invariant holds before the next point `x`
# is processed.  
# Let `H` be the current stack, `h = H[-2]`, `g = H[-1]`.  
# If `cross(h , g , x) > 0` the new point `x` is to the left of the segment
# `hg`; the polygon `[H … g , x]` is still convex and `x` must belong to the
# hull.  
# If `cross(h , g , x) ≤ 0` the point `g` is inside or on the line `hx`;
# hence it cannot be part of the convex hull of the processed points.
# Removing it preserves convexity and the invariant.  
# After all necessary removals `x` is appended, therefore the invariant holds
# after processing `x`. ∎
# 
# 
# 
# ##### Lemma 3  
# After all points are processed the stack contains exactly the vertices of the
# convex hull of `P` in counter‑clockwise order, starting with the lowest point.
# 
# **Proof.**
# 
# By Lemma&nbsp;2 the stack is always a subsequence of the current hull.
# When the algorithm terminates every point of `P` has been processed, thus
# the stack is a subsequence of the final hull `H`.  
# Conversely, every point removed from the stack was never a left turn with
# respect to its predecessor, i.e. it lay on a line segment of `H` or inside
# `H`. Therefore it cannot be a vertex of the convex hull.  
# Hence the stack contains all and only the vertices of `H`.  
# The construction of the stack visits the vertices in increasing polar angle,
# which is a counter‑clockwise walk around `H`. ∎
# 
# 
# 
# ##### Theorem  
# The algorithm outputs the set of convex hull vertices of the input points in
# counter‑clockwise order, starting with the lowest point.
# 
# **Proof.**
# 
# By Lemma&nbsp;3 the stack produced by the algorithm is exactly the vertex
# sequence of the convex hull.  
# The algorithm prints the points in this order, thus the produced output is
# correct. ∎
# 
# 
# 
...
# #### Complexity Analysis
# 
# Sorting the `n` points costs `O(n log n)` time.  
# The scanning phase inspects each point once and each point can be removed
# at most once, therefore it is `O(n)`.  
# The overall time complexity is `O(n log n)` and the memory consumption is
# `O(n)` for storing the points and the stack.
# 
# 
# 
# --------------------------------------------------------------------
...
#     """2D cross product of OA and OB vectors (signed)."""
...
#         return points.copy()
# 
#     # choose the lowest (y, then x) point as pivot
#     start = min(points, key=lambda p: (p[1], p[0]))
# 
#     # remove the pivot from the list that will be sorted
#     rest = [p for p in points if p != start]
# 
#     # sort by polar angle w.r.t. start; for equal angle by distance
...
# 
#     for p in rest[1:]:
#         # while the last three points do not make a left turn
...
#         return
# 
#     # Try to interpret first token as number of points
#     tokens = [int(x) for x in data]
#     if len(tokens) >= 3 and tokens[0] * 2 == len(tokens) - 1:
#         n = tokens[0]
#         coords = tokens[1:]
#     else:
#         # assume all tokens are coordinates
#         n = len(tokens) // 2
#         coords = tokens
# 
...
# 
#     # print as (x,y) pairs separated by a space
...
# My answer: **Solution**
# 
# For a bipartite graph `G = (L ∪ R, E)` a *matching* is a set of edges that
# does not contain two edges incident to the same vertex.
# The **maximum bipartite matching** problem asks for a matching of
# maximum cardinality.
# 
# A very common algorithm for this problem is the
# *augmenting‑path* method (also called the *Hungarian algorithm* for
# unweighted graphs).  
# It repeatedly looks for an augmenting path – a path that alternates
# between edges **not** in the current matching and edges that **are** in the
# matching – and flips the status of all edges on that path.  
# Each augmentation increases the size of the matching by one, therefore
# after no more augmenting path exists the current matching is maximum.
# 
# Because our graph is very small we can use a straightforward DFS
# implementation of the augmenting‑path algorithm.  
# It runs in `O(|L|·|R|)` time for this particular input, which is more
# than fast enough.
# 
# --------------------------------------------------------------------
# 
...
# # ------------------------------------------------------------
# # Left vertices are integers, right vertices are single‑character strings
# Edge = Tuple[int, str]
# 
# def build_adj_list(left: List[int], edges: List[Edge]) -> Dict[int, List[str]]:
#     """Build adjacency list from left vertices to right vertices."""
#     g = {u: [] for u in left}
#     for u, v in edges:
#         g[u].append(v)
#     return g
# 
# # ------------------------------------------------------------
# # 2.  Augmenting‑path DFS
# # ------------------------------------------------------------
# def try_match(u: int,
#               g: Dict[int, List[str]],
#               matchR: Dict[str, int],
#               visited: Set[str]) -> bool:
#     """DFS that tries to match left vertex `u`."""
#     for v in g[u]:
#         if v in visited:
#             continue
#         visited.add(v)
# 
#         # If the right vertex `v` is free or can be re‑matched,
#         # we match `u` to `v`.
#         if v not in matchR or try_match(matchR[v], g, matchR, visited):
#             matchR[v] = u
#             return True
#     return False
# 
# def maximum_bipartite_matching(left: List[int],
#                                edges: List[Edge]) -> int:
#     g = build_adj_list(left, edges)
#     matchR: Dict[str, int] = {}   # right vertex → left vertex
#     result = 0
# 
#     for u in left:
#         if try_match(u, g, matchR, set()):
#             result += 1
#     return result
# 
# # ------------------------------------------------------------
# # 3.  Main routine – reads the example and prints the size
# # ------------------------------------------------------------
# def solve() -> None:
#     # Example graph
...
#     size = maximum_bipartite_matching(left_vertices, edges)
# 
#     # Output only the size – as requested by the problem statement
#     print(size)
# 
# if __name__ == "__main__":
#     solve()
...
# ```
# 
# which is the maximum possible matching size for the given bipartite graph.
# 
# --------------------------------------------------------------------
# 
# **Answer:** `4`
...
# ```python
# import asyncio...
# My answer: Here is the corrected version of the program.  
# All syntax errors are fixed and the coroutine logic is preserved.
# 
# ```python
# import asyncio
# 
...
# 
# 
# async def process_data() -> float:
#     """Create several fetch tasks, wait for them all and sum the results."""
#     tasks = []
#     for i in range(1, 4):
#         task = fetch_data(i)
#         tasks.append(task)
...
# ```
# 
# The program now runs correctly: it waits for the three coroutines
# (`fetch_data(1)`, `fetch_data(2)`, `fetch_data(3)`), multiplies each delay by 2,
# and finally prints the sum of the results, which is `12`.
...
# My answer: Below is a concise implementation of **Mo’s Algorithm** that answers the
# required distinct‑element queries on the given array.
# 
# ```python
# import math
# from typing import List, Tuple
# 
# Array = List[int]
# Query = Tuple[int, int]          # inclusive left & right indices
# 
# 
# def mos_algorithm(arr: Array, queries: List[Query]) -> List[int]:
#     """
#     Return the number of distinct values for every query.
#     The algorithm runs in O((n + q)·√n).
#     """
#     n = len(arr)
#     q = len(queries)
# 
...
# 
#     # --- 2.  Pre‑process queries with indices --------------------------------
#     # Each query is stored as (block_of_left, right, original_index)
#     indexed_q = [(i // block_size, qidx, qidx) for qidx, i in enumerate(queries)]
#     # Sort by (block, right) – the classic Mo ordering.
#     indexed_q.sort(key=lambda x: (x[0], x[1]))
# 
#     # --- 3.  Data structures --------------------------------------------------
#     freq = [0] * (max(arr) + 1)   # frequency of every value in the current range
...
#             distinct -= 1
# 
#     # --- 4.  Process queries in Mo order ------------------------------------
#     cur_l, cur_r = 0, -1          # current window is empty
#     answers = [0] * q
# 
#     for blk, r, idx in indexed_q:
#         L, R = queries[idx]
# 
#         # expand / shrink right end
#         while cur_r < R:
#             cur_r += 1
#             add(cur_r)
#         while cur_r > R:
...
#     queries = [(0, 4), (2, 7), (1, 8)]   # inclusive ranges
#     counts = mos_algorithm(arr, queries)
# 
...
# 
# 1. **Block size** – we take `⌊√n⌋ + 1`.  
#    For the array of length 10 this is 4, but any block size that is
#    `O(√n)` works.
# 
# 2. **Query ordering** – the queries are sorted by the block of their left
#    endpoint, and within each block by their right endpoint
#    (the classic Mo ordering).  This guarantees that the window moves
#    only a total of `O((n + q)·√n)` steps.
# 
# 3. **Adding / removing** – a simple frequency table (`freq`) tracks how many
#    times each value appears in the current window.
#    * When an element’s count changes from 0 → 1, the number of distinct
#      elements (`distinct`) is increased.
#    * When it changes from 1 → 0, `distinct` is decreased.
# 
# 4. **Answering** – after the window `[L, R]` is adjusted to the query,
#    the current value of `distinct` is the answer for that query.
# 
# Running the program prints
# 
# ```
...
# ```
# 
# which are the counts of distinct elements for the ranges
# `(0,4)`, `(2,7)` and `(1,8)` respectively.
...
# ```
# 
# Running `meta_test()` prints:
# 
# ```
# 2
# ```
# 
# The `Database` class is a singleton, so both `db1` and `db2` share the same instance and its data list. After adding two items, the size of the shared data list is `2`.
# ==========================================
